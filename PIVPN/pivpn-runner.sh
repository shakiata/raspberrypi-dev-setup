...
sudo docker run -v $OVPN_DATA:/etc/openvpn -d -p 1194:1194/udp --cap-add=NET_ADMIN --restart always mjenz/rpi-openvpn
...
sudo docker exec -it blissful_cannon cat /etc/openvpn/pki/private/CLIENTNAME.key.XXXXPmAALe